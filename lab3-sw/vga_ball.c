 ...
 */
 ...
 /* Device registers */
 #define ball_x_coordinates(x) ((x)+0)
 #define ball_y_coordinates(x) ((x)+4)
 #define BG_RED(x) ((x)+8)
 ...
 static void write_location(vga_ball_location *location)
 {
	 iowrite32(location->ball_x, ball_x_coordinates(dev.virtbase) );
	 iowrite32(location->ball_y, ball_y_coordinates(dev.virtbase) );
 ...
 static long vga_ball_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
 {
	 ...
	 printk("Entered write loc; \n ");
 
		 if (copy_from_user(&vloc, (vga_ball_arg_l *) arg,
					sizeof(vga_ball_arg_l)))
			 return -EACCES;
		 write_location(&vloc.location);
		 printk("x = %04x, y= %04x\n", vloc.location.ball_x, vloc.location.ball_y);
		 printk("x = %04x, y= %04x\n", vloc.location.ball_x, vloc.location.ball_y);
		 break;
 
	 case VGA_BALL_READ_LOCATION:
	 printk("Entered read_loc bg; \n ");
 
		   vloc.location = dev.location;
		 if (copy_to_user((vga_ball_arg_l *) arg, &vloc,
				  sizeof(vga_ball_arg_l)))
			 return -EACCES;
		 break;
