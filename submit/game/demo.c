...
// Now also draws a 4‑tile bar (tile #23) that continuously scrolls
// from right to left across the screen without affecting existing
// gameplay features.
...
#define BAR_TILE_IDX      23    // moving bar tile

// ───── moving platforms ──────────────────────────────────────────────────────
#define MAX_PLATFORMS      3
#define PLATFORM_W      (4*32)  // 4 sprites × 32 px
...
// ───── scrolling bar ────────────────────────────────────────────────────────
#define BAR_SEGMENTS       4
#define BAR_WIDTH       (BAR_SEGMENTS*32)
#define BAR_Y              96    // vertical position of the bar
#define BAR_SPEED           4    // horizontal scroll speed (px/frame)
#define BAR_REG_BASE   (PLATFORM_REG_BASE + MAX_PLATFORMS*4) // uses regs 13‑16
...
int main(void) {
    ...
    // initialise scrolling bar
    int barX = LENGTH; // start just off‑screen to the right
    ...
    while (lives>0) {
        ...
        // ───── bar position update ──────────────────────────────────────────
        barX -= BAR_SPEED;
        if(barX < -BAR_WIDTH) barX = LENGTH; // reset once fully off‑screen
        ...
        // draw scrolling bar
        for(int k=0;k<BAR_SEGMENTS;k++){
            write_sprite_to_kernel(
                1, BAR_Y, barX + k*32,
                BAR_TILE_IDX, BAR_REG_BASE + k);
        }
        // draw chicken
        write_sprite_to_kernel(
